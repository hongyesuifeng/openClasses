# Week 1: LLM 基础与提示工程
# Introduction to Coding LLMs and AI Development

> **第 1 周：大型语言模型编程与 AI 开发导论**
> **日期**: 2025年9月22日 - 9月26日
> **主题**: LLM Prompting Playground - 核心提示词技术
> **作业**: LLM Prompting Playground

---

## 📚 本周概览

第 1 周是 CS146S 的开篇，建立对 AI 辅助开发的基础理解：

1. **LLM 的本质** - 什么是大语言模型，它是如何工作的
2. **有效提示** - 如何设计高质量的提示词
3. **核心提示技术** - 掌握 6 种关键提示方法
4. **实战练习** - 通过动手实验巩固理论

---

## 🎯 学习目标

完成本周学习后，你应该能够：

- ✅ 理解 LLM 的基本工作原理
- ✅ 掌握多种提示工程技术（K-shot、CoT、Tool Calling 等）
- ✅ 能够使用 Ollama 运行本地 LLM
- ✅ 实践 6 种不同的提示模式
- ✅ 理解每种技术的适用场景

---

## 💡 核心内容

### 一、LLM 工作原理

#### 什么是 LLM？

**LLM (Large Language Model)** 是基于深度学习的 AI 系统，通过在海量文本数据上训练，学习语言的统计规律和语义关联。

**核心特性**:
- 🧠 **自回归生成**: 基于上下文逐个 token 生成
- 👁️ **注意力机制**: 捕捉长距离依赖关系
- 📦 **上下文窗口**: 能处理的最大输入长度
- 🎯 **零样本/少样本**: 无需或仅需少量示例即可完成任务

#### Transformer 架构

现代 LLM 都基于 **Transformer** 架构，核心组件包括：

1. **Self-Attention（自注意力机制）** - 让模型关注输入序列的不同部分
2. **Multi-Head Attention（多头注意力）** - 并行学习多种注意力模式
3. **Feed-Forward Networks（前馈网络）** - 对每个位置进行非线性变换
4. **Layer Normalization（层归一化）** - 稳定训练，加速收敛

#### 训练过程

**预训练（Pre-training）**:
- 目标: 学习通用的语言理解和世界知识
- 训练数据: 网页文本、书籍、代码库、论文
- 训练任务: Next Token Prediction（下一个词预测）

**微调（Fine-tuning）**:
- Supervised Fine-Tuning (SFT): 在标注数据上继续训练
- RLHF (Reinforcement Learning from Human Feedback): 对齐人类偏好

---

### 二、Prompt Engineering（提示工程）

#### 核心原则

##### 1. 清晰性（Clarity）
提供明确的指令，避免歧义。

**❌ 糟糕的提示词**:
```
帮我写个函数
```

**✅ 优秀的提示词**:
```
请用 Python 编写一个函数，实现以下功能：
- 输入：一个整数列表
- 输出：列表中的第二大数字
- 要求：处理边界情况（空列表、单元素列表）
- 返回类型：int 或 None
```

##### 2. 上下文（Context）
提供足够的背景信息，让 LLM 理解任务环境。

**框架**:
```
【项目背景】
我们正在开发一个电商网站的后端 API...

【当前任务】
需要实现一个购物车功能...

【技术栈】
- 后端：Python + FastAPI
- 数据库：PostgreSQL
- 缓存：Redis

【具体需求】
...
```

##### 3. 示例驱动（Example-Driven）
通过 few-shot learning 提供示例。

##### 4. 思维链（Chain-of-Thought）
引导模型展示推理过程。

##### 5. 结构化（Structure）
使用清晰的结构组织提示词。

---

### 三、六大提示技术

#### 1. K-shot Prompting（K 样本提示）

**概念**: 通过提供 K 个示例来引导 LLM

**应用场景**:
- 分类任务
- 格式转换
- 模式识别

**示例**:
```
示例 1: 输入 "I love this product" → 输出 "Positive"
示例 2: 输入 "This is terrible" → 输出 "Negative"

现在: 输入 "{user_input}" → 输出 ?
```

---

#### 2. Chain-of-Thought（思维链）

**概念**: 引导 LLM 展示推理过程

**应用场景**:
- 数学问题求解
- 复杂问题推理
- 多步骤逻辑
- 代码调试

**模板**:
```
请按以下步骤思考：
1. 理解问题的核心
2. 列出可能的解决方案
3. 分析每种方案的优劣
4. 选择最优方案并说明理由
5. 给出具体实现
```

---

#### 3. Tool Calling（工具调用）

**概念**: LLM 可以调用外部工具/API 来增强其能力

**工作流程**:
1. LLM 分析用户请求
2. 决定需要调用哪些工具
3. 生成工具调用参数
4. 执行工具调用
5. 将结果返回给 LLM
6. LLM 基于结果继续处理

**应用场景**:
- 查询数据库
- 调用 API
- 执行代码
- 文件系统操作

---

#### 4. Self-Consistency Prompting（自一致性提示）

**概念**: 多次采样并通过投票选择最一致的答案

**应用场景**:
- 减少幻觉
- 提高答案可靠性
- 复杂推理任务

**流程**:
1. 对同一问题生成多个推理路径
2. 每个路径独立得出答案
3. 通过投票选择最频繁的答案

---

#### 5. RAG - Retrieval-Augmented Generation（检索增强生成）

**概念**: 结合外部知识库增强生成

**应用场景**:
- 问答系统
- 文档分析
- 知识检索

**架构**:
```
用户查询 → 检索相关文档 → 将文档作为上下文
→ LLM 基于上下文生成答案 → 返回结果
```

---

#### 6. Reflexion（反思）

**概念**: 通过自我反思迭代改进答案

**应用场景**:
- 代码生成
- 问题求解
- 写作改进

**循环**:
```
生成答案 → 自我评估 → 识别问题
→ 生成改进建议 → 重新生成 → 循环直到满意
```

---

## 💻 本周作业: LLM Prompting Playground

### 作业目标

通过实践 6 种核心提示技术，建立对 LLM 能力的直观理解。

### 技术要求

- **Python 版本**: 3.12
- **本地 LLM**: Ollama
  - 模型: `mistral-nemo:12b` 或 `llama3.1:8b`
- **开发环境**: 任何 Python IDE

### 六大提示技术实现

#### 1. K-shot Prompting
- **文件**: `k_shot_prompting.py`
- **任务**: 实现少样本分类

#### 2. Chain-of-Thought
- **文件**: `chain_of_thought.py`
- **任务**: 实现思维链推理

#### 3. Tool Calling
- **文件**: `tool_calling.py`
- **任务**: 实现工具调用功能

#### 4. Self-Consistency Prompting
- **文件**: `self_consistency_prompting.py`
- **任务**: 实现自一致性验证

#### 5. RAG
- **文件**: `rag.py`
- **任务**: 实现检索增强生成

#### 6. Reflexion
- **文件**: `reflexion.py`
- **任务**: 实现自我反思循环

### 评分标准

- **总分**: 60 分
- **每项技术**: 10 分
  - 实现正确性 (5 分)
  - 代码质量 (3 分)
  - 文档说明 (2 分)

---

## 🔧 环境设置

### 安装 Ollama

#### macOS / Linux
```bash
curl -fsSL https://ollama.com/install.sh | sh
```

#### Windows
下载安装程序: https://ollama.com/download

### 拉取模型

```bash
# 推荐模型
ollama pull mistral-nemo:12b
ollama pull llama3.1:8b

# 测试运行
ollama run mistral-nemo:12b "Hello, how are you?"
```

### Python 依赖

```bash
# 创建虚拟环境
python -m venv venv
source venv/bin/activate  # Windows: venv\Scripts\activate

# 安装依赖
pip install ollama openai anthropic
```

---

## 📚 本周阅读材料

### 详细 Readings

Week 1 提供了 3 个详细的 reading 文档（可在 `readings/` 目录找到）：

1. **Deep Dive into LLMs** (01-deep-dive-into-llms.md)
   - LLM 的架构、训练过程、推理机制
   - 能力与局限性分析
   - 软件开发中的应用

2. **Prompt Engineering Overview** (02-prompt-engineering-overview.md)
   - 提示工程的定义和价值
   - 5 大核心原则
   - 6 种常见模式
   - 常见错误和避免方法

3. **Prompt Engineering Guide** (03-prompt-engineering-guide.md)
   - 5 个即用提示词模板
   - 2 个实际场景案例
   - 5 个调优技巧
   - A/B 测试方法

### 阅读建议

- 📖 **阅读顺序**: 按编号顺序阅读，逐步深入
- ⏱️ **预计时间**: 6-8 小时
- 💡 **重点**: 在阅读时思考每种技术如何应用于实际编程场景

---

## 🎓 核心思想

### 1. LLM is a Probability Model

LLM 基于概率生成下一个 token，理解这一点有助于：
- 设置合理的期望
- 设计更好的提示词
- 理解输出的不确定性

### 2. Context is Everything

LLM 的效果取决于提供的上下文质量：
- 清晰的项目结构
- 完善的文档
- 明确的需求
- 一致的代码风格

### 3. Iterative Refinement

不要期望一次性得到完美结果：
- 快速迭代
- 持续验证
- 及时反馈

### 4. Trust but Verify

AI 会犯错（幻觉问题），必须：
- 建立验证机制
- 代码审查
- 测试覆盖

---

## 📊 学习建议

### 推荐学习流程

1. **预习阶段** (2-3 小时)
   - 快速浏览所有 readings
   - 标记不理解的概念
   - 准备问题清单

2. **课堂参与** (2 小时)
   - 周一讲座: LLM 原理
   - 周五讲座: 提示工程
   - 积极提问和讨论

3. **实践阶段** (6-8 小时)
   - 按顺序实现 6 种技术
   - 每种技术都进行充分测试
   - 记录实验结果和观察

4. **反思总结** (1-2 小时)
   - 总结每种技术的适用场景
   - 比较不同技术的优缺点
   - 思考如何应用于实际项目

### 学习技巧

- 🔄 **迭代实验**: 不要期望一次得到完美结果
- 📝 **详细记录**: 记录每次尝试的参数和结果
- 🤔 **对比分析**: 比较不同提示词的效果差异
- 🎯 **场景化**: 思考每种技术在什么场景下最有效

---

## ✅ 本周检查清单

### 课前准备
- [ ] 安装 Ollama
- [ ] 拉取至少一个 LLM 模型
- [ ] 设置 Python 开发环境
- [ ] 阅读完所有 readings

### 课后实践
- [ ] 实现 K-shot Prompting
- [ ] 实现 Chain-of-Thought
- [ ] 实现 Tool Calling
- [ ] 实现 Self-Consistency Prompting
- [ ] 实现 RAG
- [ ] 实现 Reflexion
- [ ] 完成 README.md
- [ ] 完成实验报告

### 自我评估
- [ ] 理解 LLM 的基本原理
- [ ] 能够独立选择合适的提示技术
- [ ] 能够设计有效的提示词
- [ ] 能够分析提示效果并进行优化

---

## 🔗 学习资源

### 官方资源
- [课程官网](https://themodernsoftware.dev/)
- [作业仓库](https://github.com/mihail911/modern-software-dev-assignments)
- [Ollama 文档](https://ollama.com/docs)

### 推荐阅读
- [Prompt Engineering Guide](https://www.promptingguide.ai/)
- [Anthropic Prompt Library](https://docs.anthropic.com/claude/prompt-library)

### 工具
- [Ollama](https://ollama.com/)
- [OpenAI Playground](https://platform.openai.com/playground)
- [Anthropic Console](https://console.anthropic.com/)

---

## ❓ 常见问题

### Q1: 为什么要用本地 LLM 而不是 API？

**A**: 本地 LLM 提供以下优势：
- 💰 **成本**: 无 API 调用费用
- 🔒 **隐私**: 数据不离开本地机器
- 🎮 **自由**: 可以自由实验和调试
- 📚 **学习**: 更深入理解 LLM 工作原理

### Q2: 如何判断哪种提示技术最适合我的任务？

**A**: 决策树：
```
需要外部信息？
├─ 是 → RAG
└─ 否
    ├─ 需要多步推理？
    │   ├─ 是 → Chain-of-Thought + Self-Consistency
    │   └─ 否
    │       ├─ 有标准格式？
    │       │   ├─ 是 → K-shot Prompting
    │       │   └─ 否 → Simple Prompt
    └─ 需要执行操作？
        └─ 是 → Tool Calling
    └─ 需要迭代改进？
        └─ 是 → Reflexion
```

### Q3: 提示词需要多长？

**A**: 取决于任务复杂度：
- **简单任务**: 1-2 句话即可
- **中等任务**: 5-10 句话，包含示例
- **复杂任务**: 完整的结构化文档

**原则**: 清晰 > 简洁，但也避免冗余

---

## 🚀 下周预告

**Week 2: The Anatomy of Coding Agents**
- 🤖 探索编码代理的架构
- 🔧 实现一个完整的 LLM 应用
- 🎯 学习如何在实际项目中集成 LLM

**准备工作**:
- 熟悉 FastAPI 框架
- 了解基本的数据库操作
- 复习本周的提示技术

---

**Week 1 是整个课程的基础，掌握好这些知识将为后续学习打下坚实基础！** 💪
