...
> 《甜心迷宫》— Q版卡通卡牌 Roguelike 游戏，使用 Godot 4.x 开发。
> 玩家扮演魔法少女，挑战云端魔法学院的甜心迷宫塔楼，对战可爱怪物，构筑卡牌牌组。
...
**执行任务时一口气完成所有文件修改，不要中途停下来问问题或要求确认。用你最好的判断直接推进。** 只有在真正阻塞（如缺少关键依赖、发现严重矛盾）时才停下来。

适用于：多文件编辑、批量 UI 调整、跨场景主题改造、测试修复等所有开发任务。
