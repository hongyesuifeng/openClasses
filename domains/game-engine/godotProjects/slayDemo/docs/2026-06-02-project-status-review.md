...
> 分析日期: 2026-06-02（最后更新: 2026-06-02，含 M5 存档 + 经济曲线 + 随机地图修正）  
...
- [x] **事件节点选牌 UI**：`event_scene.gd` 已实现 `_render_card_selection()`，玩家可手动点选目标卡牌，含取消按钮
- [x] **地图路线随机生成**：`map_generator.gd` 随机 DAG 算法已完整实现，`run_controller.gd` 默认 `use_generated_map = true`，新游戏每局路线不同
- [x] **正式经济曲线**：战斗金币含楼层加成（`floor_index * 1.5`）；商店价格含楼层加成（`floor_index * 3`）；删牌价格递增（`75 + removal_count * 25`）
- [x] **存档系统（M5）**：`save_service.gd` 完整实现 save/load/restore/delete；主菜单"继续游戏"按钮根据存档动态显示

**P1（仍待处理）：**

- [ ] **精英遗物奖励 UI 呈现**：遗物奖励仅打 print 日志，玩家无明确感知
...
├── run_controller.gd   # run 推进控制（含随机地图生成 + 自动存档）
├── save_service.gd     # 存档：save / load / restore / delete
...
**测试体系**：20+ 测试脚本，当前 **253 断言通过**（单元测试 + 集成测试）
...
- [ ] **精英遗物奖励 UI**：获得遗物时展示选择/确认界面，当前靠 print 日志反馈
- [ ] **商店遗物商品**：扩展 `shop_service.gd`，支持遗物上架
...
```
策划完成度  ████████████████████░░  约 90%
           核心玩法设计完整，精英奖励 UI/药水待细化

开发完成度  ██████████████████░░░░  约 85%
           核心系统+存档+随机地图+经济曲线完整，精英奖励 UI/音频/动画待做
...
| 优先级 | 工作 | 视角 | 说明 |
|--------|------|------|------|
| 1 | **精英遗物奖励 UI** | 开发 + 美术 | 当前靠 print 日志，玩家无感知；遗物图标资源已有 |
| 2 | **商店遗物商品** | 开发 | 扩展 `shop_service.gd`，让遗物进入商店流通 |
| 3 | **状态图标补全** | 美术 | 力量/中毒/易伤/虚弱等状态图标缺失，影响战斗可读性 |
| 4 | **音效接入** | 开发 + 美术 | `card_place_1.ogg` + `card_slide_1.ogg` 已有，接入 AudioManager 即可提升手感 |
