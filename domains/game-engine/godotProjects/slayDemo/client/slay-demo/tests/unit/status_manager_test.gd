...
var status: RefCounted  # 使用 RefCounted 基类避免类型加载问题
...
func _test_apply_status(ctx: Variant) -> void:
	_before_each()
	ctx.assert_eq(status.call("get_stacks", "strength"), 0, "初始力量为 0")
	status.call("apply_status", "strength", 3)
	ctx.assert_eq(status.call("get_stacks", "strength"), 3, "施加 3 层力量")
	status.call("apply_status", "strength", 2)
	ctx.assert_eq(status.call("get_stacks", "strength"), 2, "力量更新到 2")


func _test_remove_status(ctx: Variant) -> void:
	_before_each()
	status.call("apply_status", "vulnerable", 2)
	ctx.assert_true(status.call("has_status", "vulnerable"), "施加后存在易伤")
	status.call("remove_status", "vulnerable")
	ctx.assert_false(status.call("has_status", "vulnerable"), "移除后不存在易伤")
	ctx.assert_eq(status.call("get_stacks", "vulnerable"), 0, "移除后易伤层数为 0")


func _test_tick_turn_end_decrements_stacks(ctx: Variant) -> void:
	_before_each()
	status.call("apply_status", "vulnerable", 3)
	ctx.assert_eq(status.call("get_stacks", "vulnerable"), 3, "初始易伤层数为 3")
	status.call("tick_turn_end")
	ctx.assert_eq(status.call("get_stacks", "vulnerable"), 2, "易伤层数 -1")
	status.call("tick_turn_end")
	ctx.assert_eq(status.call("get_stacks", "vulnerable"), 1, "易伤层数再次 -1")
	status.call("tick_turn_end")
	ctx.assert_false(status.call("has_status", "vulnerable"), "易伤消失")


func _test_tick_turn_start_poison(ctx: Variant) -> void:
	_before_each()
	status.call("apply_status", "poison", 5)
	var result: Dictionary = status.call("tick_turn_start")
	ctx.assert_eq(result.get("poison_damage", 0), 5, "中毒造成 5 点伤害")
	ctx.assert_eq(status.call("get_stacks", "poison"), 4, "中毒层数 -1")


func _test_tick_turn_start_regeneration(ctx: Variant) -> void:
	_before_each()
	status.call("apply_status", "regeneration", 3)
	var result: Dictionary = status.call("tick_turn_start")
	ctx.assert_eq(result.get("regeneration_heal", 0), 3, "生命回复恢复 3 点")
	ctx.assert_eq(status.call("get_stacks", "regeneration"), 2, "生命回复层数 -1")
...
func _test_calculate_damage_with_strength(ctx: Variant) -> void:
	...
	# 攻击者有力量
	status.call("apply_status", "strength", 3)
	var damage: int = status.call("calculate_damage", 6, true)
...
func _test_calculate_damage_with_vulnerable(ctx: Variant) -> void:
	...
	# 被攻击者有易伤
	status.call("apply_status", "vulnerable", 2)
	var damage: int = status.call("calculate_damage", 10, false)
...
func _test_calculate_damage_with_weak(ctx: Variant) -> void:
	_before_each()
	# 攻击者有无力
	status.call("apply_status", "weak", 2)
	var damage: int = status.call("calculate_damage", 10, true)
...
func _test_calculate_damage_combined(ctx: Variant) -> void:
	...
	# 攻击者有力量和无力
	status.call("apply_status", "strength", 2)
	status.call("apply_status", "weak", 1)
	var damage: int = status.call("calculate_damage", 8, true)
...
func _test_calculate_block_with_dexterity(ctx: Variant) -> void:
	_before_each()
	status.call("apply_status", "dexterity", 2)
	var block: int = status.call("calculate_block", 5)
	ctx.assert_eq(block, 7, "5 + 2 敏捷 = 7")


func _test_calculate_block_with_frail(ctx: Variant) -> void:
	_before_each()
	status.call("apply_status", "frail", 2)
	var block: int = status.call("calculate_block", 8)
	ctx.assert_eq(block, 6, "floor(8 * 0.75) = 6")


func _test_on_hit_thorns(ctx: Variant) -> void:
	_before_each()
	status.call("apply_status", "thorns", 3)
	var damage: int = status.call("on_hit")
...
func _test_permanent_status_not_decremented(ctx: Variant) -> void:
	...
	status.call("apply_status", "strength", 3)
	status.call("tick_turn_end")
	ctx.assert_eq(status.call("get_stacks", "strength"), 3, "力量不随回合减少")
...
func _test_get_snapshot(ctx: Variant) -> void:
	...
	status.call("apply_status", "strength", 2)
	status.call("apply_status", "vulnerable", 3)
	var snapshot: Array = status.call("get_snapshot")
