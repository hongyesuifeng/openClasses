...
> 本章介绍 AI Agent 的基本概念、核心特征及其在游戏开发领域的独特价值。理解这些基础概念是后续章节学习的前提。
...
> 本章详细拆解 Agent 的五大核心组件：感知、规划、行动、反思和记忆。理解这些组件如何协同工作，是构建有效 Agent 系统的基础。
...
> 本章介绍三种主流的 Agent 实现模式，以及 Andrew Ng 提出的四大 Agentic 设计模式。不同的模式适用于不同的场景，实际项目中常常组合使用。
...
> 本章通过一个完整的实战案例——游戏小镇，展示 Multi-Agent 协作系统的设计与实现。该案例模拟了一个 4 人游戏开发团队，演示了 Agent 如何在真实场景中进行角色扮演和协作决策。
...
> 本章探讨 AI Agent 在游戏开发各环节的实际应用，包括 NPC 智能化、游戏测试、内容生成和辅助开发等场景。每个场景都配有具体的实现思路和案例分析。
...
> 本章深入 Agent 系统的技术实现细节，包括记忆系统、决策系统、工具调用的具体实现方案。这些内容对实际开发有直接的参考价值。
...
> 本章总结 Agent 系统开发的最佳实践，包括角色定义、提示词设计、安全边界、评估优化等关键主题。遵循这些实践可以避免常见陷阱，提高 Agent 系统的成功率。
...
| **Andrew Ng: Agentic AI** | 吴恩达最新课程，四大设计模式详解（Reflection、Tool Use、Planning、Multi-Agent） | [Datawhale 中文版](https://github.com/datawhalechina/agentic-ai) |
...
|:---|:---|:---|
| **创意瓶颈** | 多 Agent 模拟团队头脑风暴 | 突破思维局限，激发创新 |
